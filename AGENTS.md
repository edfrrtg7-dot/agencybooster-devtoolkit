...
esbuild owns the dist/ directory. Running bare `tsc` would try to emit .d.ts
...
7. **Content script** — `content.ts` runs in page context. Communicates
   with background via `chrome.runtime.sendMessage` only.
8. **Content script is minimal** — no classes, no toolkit imports. It is
   a thin message bridge. Do NOT add module logic here.
9. **Entrypoint separation** — `core/` and `modules/` are only imported
...
- `bootstrap.ts` imports every module directly. Adding a module requires
  editing this file. This is acceptable at 6 modules. If module count
  grows beyond ~10, switch to a dynamic registry pattern.
- `config.ts` `DEFAULT_CONFIG.modules` hard-codes module IDs. Same
  coupling. Keep IDs in sync with module class `id` fields.
